                    ...
                    <div class="relative h-64 overflow-hidden">
                        ...
                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                            <div class="flex space-x-3">
                                <button class="bg-white/20 backdrop-blur-sm text-white px-4 py-2 rounded-lg hover:bg-white/30 transition-colors duration-200" onclick="openVehicleModal('{{ $vehicle->id }}', '{{ $vehicle->make }}', '{{ $vehicle->model }}', '{{ $vehicle->year }}', '{{ $vehicle->color }}', '{{ $vehicle->car_type }}', '{{ $vehicle->transmission }}', '{{ $vehicle->fuel_type }}', '{{ $vehicle->daily_rate }}', '{{ $vehicle->availability }}', '{{ $vehicle->mileage }}', '{{ $vehicle->seating_capacity }}', '{{ $vehicle->insurance }}', '{{ $vehicle->last_service }}', '{{ $vehicle->condition }}', '{{ json_encode($vehicle->images ?? []) }}')">
